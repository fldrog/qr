...
public class MailTask implements Runnable {
	...
	private MimeMessage composeMessage(Session session, String mail2)
		...
		message.setFrom(new InternetAddress(Conf.getEnv().getProperty(
				"email.from")));
		message.setReplyTo(new InternetAddress[] { new InternetAddress(Conf
				.getEnv().getProperty("email.from")) });
